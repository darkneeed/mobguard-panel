...
class NetworkAnalyzer:
    ...
    async def check_is_mobile(self, ip: str, uuid: str = None, tag: str = None) -> DecisionBundle:
        ...
        def finalize(verdict: str, confidence: str, details: str) -> DecisionBundle:
            ...
            bundle.log = list(log)
            if verdict == 'HOME' and CONFIG['settings'].get('auto_enforce_requires_hard_or_multi_signal', True):
        ...
        log.append(f"[ANALYSIS] Starting analysis for IP {ip}")

        manual_decision = await platform_store.async_get_ip_override(ip)
        if not manual_decision:
            manual_decision = await db.get_unsure_pattern(ip)
        if manual_decision:
            bundle.source = "manual_override"
            log.append(f"[ANALYSIS] Manual override: {manual_decision}")
            ...
            if manual_decision == 'MOBILE':
                return finalize('MOBILE', 'HIGH_MOBILE', "Manual MOBILE")
            if manual_decision == 'HOME':
                bundle.punitive_eligible = True
                return finalize('HOME', 'HIGH_HOME', "Manual HOME")
            return finalize('SKIP', 'SKIP', "Manual SKIP")
        log.append("[ANALYSIS] No manual override found")

        log.append(" Querying IPInfo API")
        ipinfo_data = await ipinfo_api.get_ip_info(ip)
        org = ipinfo_data.get('org', '')
        hostname = ipinfo_data.get('hostname', '')
        asn = ipinfo_api.parse_asn(org)
        isp_name = ipinfo_api.normalize_isp_name(org)
        bundle.asn = asn
        bundle.isp = isp_name
        bundle.details = isp_name

        log.append(f" IPInfo result: ASN {asn}, ISP '{isp_name}'")
        if hostname:
            log.append(f" Hostname: {hostname}")

        if ipinfo_api.is_datacenter(org, hostname):
            penalty = -100
            score += penalty
            log.append(" ✗ DATACENTER detected → Hard block")
            bundle.add_reason(
                code="datacenter",
                source="datacenter",
                weight=penalty,
                ...
                metadata={"org": org, "hostname": hostname},
            )
            self._record_stats(asn, 'HOME', None, org)
            return finalize('HOME', 'HIGH_HOME', f"{isp_name} (Datacenter)")
        log.append(" Not a datacenter")

        log.append(" ASN classification")
        if asn in CONFIG.get('pure_home_asns', []):
            penalty = CONFIG['settings'].get('pure_home_asn_penalty', -100)
            score += penalty
            log.append(f" ✗ Pure HOME ASN {asn} ({penalty} points)")
            bundle.add_reason(
                code="pure_home_asn",
                source="asn_home",
                weight=penalty,
                kind="hard",
                direction="HOME",
                message=f"Pure HOME ASN {asn}",
                metadata={"asn": asn},
            )
            self._record_stats(asn, 'HOME', None, org)
            return finalize('HOME', 'HIGH_HOME', f"{isp_name} (Pure HOME ASN)")
        if asn in CONFIG.get('pure_mobile_asns', []):
            bonus = CONFIG['settings'].get('pure_asn_score', 60)
            score += bonus
            log.append(f" ✓ Pure MOBILE ASN {asn} (+{bonus})")
            bundle.add_reason(
                ...
                kind="soft",
                direction="MOBILE",
                message=f"Pure MOBILE ASN {asn}",
                metadata={"asn": asn},
            )
        elif asn in CONFIG.get('mixed_asns', []):
            mixed_score = CONFIG['settings'].get('mixed_asn_score', 45)
            score += mixed_score
            log.append(f" ⚖️ Mixed ASN {asn} (+{mixed_score})")
            bundle.add_reason(
                code="mixed_asn",
                source="asn_mixed",
                weight=mixed_score,
                kind="soft",
                direction="MOBILE",
                message=f"Mixed ASN {asn}",
                metadata={"asn": asn},
            )
        else:
            log.append(f" Unknown ASN {asn} (no bonus)")
        log.append(f" Score after ASN: {score}")

        log.append(" Keyword analysis")
        home_keywords = CONFIG.get('home_isp_keywords', [])
        mobile_keywords = CONFIG.get('allowed_isp_keywords', [])
        found_home_kw = [kw for kw in home_keywords if kw in f"{isp_name} {hostname}".lower()]
        found_mobile_kw = [kw for kw in mobile_keywords if kw in f"{isp_name} {hostname}".lower()]

        if found_home_kw:
            penalty = CONFIG['settings'].get('ptr_home_penalty', -20)
            score += penalty
            log.append(f" ✗ HOME keywords found: {found_home_kw} ({penalty})")
            bundle.add_reason(
                code="keyword_home",
                source="keyword",
                weight=penalty,
                kind="soft",
                direction="HOME",
                message=f"HOME keywords found: {found_home_kw}",
                metadata={"keywords": found_home_kw},
            )
        else:
            log.append(" No HOME keywords")

        if found_mobile_kw:
            bonus = CONFIG['settings'].get('mobile_kw_bonus', 20)
            score += bonus
            log.append(f" ✓ MOBILE keywords found: {found_mobile_kw} (+{bonus})")
            bundle.add_reason(
                code="keyword_mobile",
                source="keyword",
                weight=bonus,
                kind="soft",
                direction="MOBILE",
                message=f"MOBILE keywords found: {found_mobile_kw}",
                metadata={"keywords": found_mobile_kw},
            )
        else:
            log.append(" No MOBILE keywords")
        log.append(f" Score after keywords: {score}")

        log.append(" Behavioral analysis")
        concurrency_immunity = False
        if uuid and tag:
            behavior = await behavioral_engine.analyze(uuid, ip, tag)
            for b_log in behavior['logs']:
                log.append(f" {b_log}")
            score += behavior['total_behavior_score']
            concurrency_immunity = behavior['concurrency_immunity']
            bundle.signal_flags['concurrency_immunity'] = concurrency_immunity
            ...
            if behavior['subnet_bonus'] > 0:
                bundle.add_reason(
                    code="behavior_subnet_mobile",
                    source="behavior_subnet",
                    weight=behavior['subnet_bonus'],
                    kind="soft",
                    direction="MOBILE",
                    message="Subnet has prior mobile evidence",
                    metadata={"subnet": db.get_subnet(ip)},
                )
            elif behavior['subnet_bonus'] < 0:
                bundle.add_reason(
                    code="behavior_subnet_home",
                    source="behavior_subnet",
                    weight=behavior['subnet_bonus'],
                    kind="soft",
                    direction="HOME",
                    message="Subnet has prior home evidence",
                    metadata={"subnet": db.get_subnet(ip)},
                )
        else:
            log.append(" Skipped (no uuid/tag provided)")
        log.append(f" Score after behavioral: {score}")

        log.append(" Machine learning check")
        if asn:
            combo_key = ""
            combo_keywords = found_home_kw + found_mobile_kw
            if combo_keywords:
                combo_key = f"{asn}+{','.join(sorted(set(combo_keywords)))}"

            promoted_combo = await platform_store.async_get_promoted_pattern('combo', combo_key) if combo_key else None
            promoted_asn = await platform_store.async_get_promoted_pattern('asn', str(asn))
            ...
            if promoted_combo:
                ...
                log.append(
                    f" ✓ Promoted combo '{combo_key}' → {promoted_combo['decision']} ({learned_weight})"
                )
                bundle.add_reason(
                    code="learning_combo",
                ...
                score += learned_weight
                log.append(
                    f" ✓ Promoted ASN {asn} → {promoted_asn['decision']} ({learned_weight})"
                )
                bundle.add_reason(
                    code="learning_asn",
                    source="learning_asn",
                    ...
                    kind="soft",
                    direction=direction,
                    message=f"Promoted ASN pattern {asn}",
                    metadata={
                        "asn": asn,
                ...
                mobile_conf = await db.get_learning_confidence('asn', str(asn), 'MOBILE')
                ...
                if mobile_conf > home_conf and mobile_conf >= 3:
                    ...
                    score += bonus
                    log.append(f" ✓ Legacy ML: ASN {asn} → MOBILE (+{bonus}, {mobile_conf} cases)")
                    bundle.add_reason(
                        ...
                        kind="soft",
                        direction="MOBILE",
                        message=f"Legacy ASN learning {asn}",
                        metadata={"asn": asn, "support": mobile_conf},
                    )
                elif home_conf > mobile_conf and home_conf >= 3:
                    penalty = -min(home_conf, 4)
                    score += penalty
                    log.append(f" ✗ Legacy ML: ASN {asn} → HOME ({penalty}, {home_conf} cases)")
                    bundle.add_reason(
                        code="legacy_learning_asn",
                        source="legacy_learning_asn",
                        weight=penalty,
                        ...
                        direction="HOME",
                        message=f"Legacy ASN learning {asn}",
                        metadata={"asn": asn, "support": home_conf},
                    )
                else:
                    log.append(f" ML: No strong pattern (M:{mobile_conf}, H:{home_conf})")
        else:
            log.append(" Skipped (no ASN)")
        log.append(f" Score after ML: {score}")

        threshold_home = CONFIG['settings'].get('threshold_home', 15)
        threshold_mobile = CONFIG['settings'].get('threshold_mobile', 60)
        log.append(" Fallback ip-api check")
        if asn in CONFIG.get('mixed_asns', []) and threshold_home < score < threshold_mobile:
            log.append(f" Mixed ASN in UNSURE zone ({threshold_home}<{score}<{threshold_mobile})")
            log.append(" Calling ip-api.com...")
            api_mobile = await self._check_ip_api(ip)
            if api_mobile is True:
                bonus = CONFIG['settings'].get('ip_api_mobile_bonus', 30)
                score += bonus
                log.append(f" ✓ ip-api: mobile=true (+{bonus})")
                bundle.add_reason(
                    code="ip_api_mobile",
                    source="ip_api",
                    weight=bonus,
            ...
            elif api_mobile is False:
                log.append(" ⚠️ ip-api: mobile=false (ignored)")
            else:
                log.append(" ip-api: no data")
        else:
            if asn not in CONFIG.get('mixed_asns', []):
                log.append(" Skipped (not Mixed ASN)")
            else:
                log.append(f" Skipped (score {score} outside UNSURE zone)")
        log.append(f" Score after fallback: {score}")

        log.append(" Computing final verdict")
        threshold_probable_mobile = CONFIG['settings'].get('threshold_probable_mobile', 50)
        threshold_probable_home = CONFIG['settings'].get('threshold_probable_home', 30)

        if concurrency_immunity and score < threshold_mobile:
            log.append(" ⚡ CGNAT immunity active → HOME blocked")
            bundle.add_reason(
                code="cgnat_immunity",
                source="concurrency",
                weight=0,
            ...
            verdict = 'UNSURE'
            confidence = 'UNSURE'
        elif score >= threshold_mobile:
            verdict = 'MOBILE'
            confidence = 'HIGH_MOBILE'
            log.append(f" ✓ Score {score} >= {threshold_mobile} → HIGH_MOBILE")
        elif score >= threshold_probable_mobile:
            verdict = 'MOBILE'
            confidence = 'PROBABLE_MOBILE'
            log.append(f" ✓ Score {score} >= {threshold_probable_mobile} → PROBABLE_MOBILE")
        elif score <= threshold_home:
            verdict = 'HOME'
            confidence = 'HIGH_HOME'
            log.append(f" ✗ Score {score} <= {threshold_home} → HIGH_HOME")
        elif score <= threshold_probable_home:
            verdict = 'HOME'
            confidence = 'PROBABLE_HOME'
            log.append(f" ✗ Score {score} <= {threshold_probable_home} → PROBABLE_HOME")
        else:
            verdict = 'UNSURE'
            confidence = 'UNSURE'
            log.append(f" ? Score {score} in UNSURE zone ({threshold_probable_home}-{threshold_probable_mobile})")

        log.append(f" FINAL: {verdict} ({confidence}, score: {score})")
        finalized = finalize(verdict, confidence, f"{isp_name} (Score {score})")

        if uuid and verdict in ('MOBILE', 'HOME'):
            await behavioral_engine.record_decision(ip, uuid, verdict)

        self._record_stats(asn, verdict, None, org)
        return finalized
