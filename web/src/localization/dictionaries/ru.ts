...
export const ruDictionary: TranslationDictionary = {
  ...
  modules: {
    eyebrow: "Fleet",
    title: "Управляемые из панели MobGuard Module",
    description: "Создавайте карточки модулей в панели, получайте install bundle и отслеживайте первое подключение collector-ноды.",
    count: "{count} модулей",
    loadFailed: "Не удалось загрузить список модулей",
    listTitle: "Подготовленные модули",
    listDescription: "Сначала создайте карточку, затем подключите collector через сгенерированный install bundle.",
    ...
    createDescription: "Сначала заполните deployment-метаданные. После сохранения панель сгенерирует module_id и API token.",
    detailsTitle: "Детали модуля",
    detailsDescription: "Редактируйте deployment-метаданные и обновляйте install preview без изменения runtime-протокола.",
    ...
    installDescription: "Поток повторяет подход Remnawave: скопируйте compose, раскройте token, запустите ноду и дождитесь первого heartbeat.",
    ...
    fields: {
      ...
      host: "Хост или адрес",
      port: "Порт ноды",
      accessLogPath: "Путь к access log",
      configProfiles: "Имена config profile",
      provider: "Провайдер",
      notes: "Заметки"
    },
    cards: {
